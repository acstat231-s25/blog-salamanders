...
3. [Top Artists](yes-another-page.qmd) conveys whether certain BPMs, song durations, years, and artists appear more frequently in Rolling Stone’s 500 Greatest Songs of All Time Ranking.

# Data Collection

On April 14, we collected data from an [online music database](https://cs.uwaterloo.ca/~dtompkin/music/list/Best9.html) created by Dave Tompkins, a professor at the University of Waterloo in Canada. The initial dataset we scraped contains 7 variables:
...
We used the following new (aside from those used during class) RStudio packages that we used during the creative process for our visualizations: 

@ggwordcloud: allows us to create word frequency word clouds, incorporating `ggplot` and `geom_text_wordcloud` functions 
<br> @xfun: provides helper functions to support other packages such as `knitr,` which is another common package used in our project
...
Furthermore, our data does not represent the average song released throughout from the 1950s to 2000s. Our data was based on a subjective list of the best songs of all time, so it likely doesn't represent overall trends in all music lyrics and other data. Additionally, the rock genre was overly represented in our data, consisting of more than 40% of the songs analyzed. Thus, our findings cannot be generalized to all songs as a whole, as our data was not representative of the population of songs in the first place.

# References

- Rolling Stone Magazine. "The 500 Greatest Songs of All Time."

- Tompkins, Dave. Online Music Database. University of Waterloo.

- Pixabay License - Record Image
